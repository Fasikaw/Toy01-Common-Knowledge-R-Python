...
capitalized.
 ...
 - ### 2. Instances
   - instance = `object` 
   - Instances are created from classes. They are concrete objects created based on the "blue-print"(class), and each of which is totally independent object. "class" is like a **catagory**. If "class" is an abstract representation of a certain object, while "instance" is a concrete example of a certain object develped from the "class".
   ...
   - 1> Let's define the **instance attributes** in the class. First, think about what features are supposed to vary by each object(instance). If the category is about 'house', then 'price' might vary! Ok! then build the **instance attribute** of 'price' inside of the "Constructor". The constructor initializes the **instance attributes**.   
     ```
     class House:
         def __init__(self, price_param, roomsize_param, material_param):
             self.price = price_param
             self.roomsize = roomsize_param
             self.material = material_param
             self.architect = "Minkun always"
     ```
     - [Note]: what is "self"? It's referring to the instance! 
   - 2> Let's create instances. 
